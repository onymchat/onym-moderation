...
SECRET_KEY="$REPO_ROOT/secrets/devicecheck.p8"
...
if [ ! -f "$SECRET_KEY" ]; then
    warn "No secrets/devicecheck.p8 — the gate will refuse every device (checkRequired)."
    warn "Copy your AuthKey_<KEYID>.p8 there to enable enforcement."
fi
...
DROPLET_NAME="onym-moderation-apple"
...
REMOTE_DIR="/opt/onym-moderation-apple"
...
if [ -f "$SECRET_KEY" ]; then
    scp "${SSH_OPTS[@]}" "$SECRET_KEY" "$REMOTE:$REMOTE_DIR/secrets/devicecheck.p8" >/dev/null
    ssh "${SSH_OPTS[@]}" "$REMOTE" "chmod 600 $REMOTE_DIR/secrets/devicecheck.p8"
fi
ok "Secrets in place"

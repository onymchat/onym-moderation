...
//! proof that the vendor wrote faithfully (Moderation-Device-Recall.md
//! §4 requirement 2, §8 gap 9). Every `deviceRecall:write` call is
//! recorded against the verdict hash (or the expiry/deadline rule) that
...
pub struct WriteLogEntry {
    ...
    pub case_open: bool,
    pub banned: bool,
    /// JSON of the values the write request actually specified, e.g.
    /// `{"bitSecond":true}` — the only-changed-values audit trail.
    pub fields_written: String,
    pub outcome: String,
...
impl Store {
    ...
    /// Create the schema. This crate has shipped no earlier schema, so
    /// there is nothing to upgrade yet — but the trap is inherited and
    /// real: `CREATE TABLE IF NOT EXISTS` is a **no-op** against a
    /// table that already exists, so the FIRST schema change after a
    /// deployment must add an explicit column-migration path (see
    /// `apple/src/store.rs`'s `add_column` + `backfill_decided_at` for
    /// the shape, and note that backfilled values can decide the
    /// causal fold — a placeholder silently reorders history).
    fn migrate(&self) -> Result<(), Error> {
        ...
        conn.execute_batch(
            ...
            -- Backend-issued challenges, folded into the signed payload
            -- and Play Integrity's requestHash. Single-use (claim sets
            -- consumed_at atomically) and short-lived; without them a
            -- client-chosen timestamp would be the only freshness bound
            -- on a token, and the requestHash would bind nothing the
            -- server chose.
            ...
            -- The last recall write Google accepted per binding, so
            -- reconciliation can recognize its own just-executed state
            -- behind Google's documented up-to-30s write-to-read
            -- propagation lag rather than re-writing it every session
            -- (profile §4 requirement 5).
            CREATE TABLE IF NOT EXISTS recall_writes (
                device_binding TEXT PRIMARY KEY,
            ...
            -- `case_open`/`banned` record the intended state after the
            -- write; `fields_written` records which values the request
            -- actually specified, proving the only-changed-values
            -- discipline (profile §4 requirement 3) to an auditor.
    ...
    /// Record a freshly issued challenge. Expired rows are swept on the
    /// way past so the table stays bounded.
    pub fn issue_challenge(
        ...
        conn.execute(
            "INSERT INTO challenges (challenge, purpose, issued_at, expires_at)
             VALUES (?1, ?2, ?3, ?4)",
            params![challenge, purpose, now, expires_at],
        )?;
        Ok(())
    }

    /// Unexpired, unconsumed challenges on hand — the issue path's
    /// size cap consults this so an unauthenticated caller cannot grow
    /// the table faster than the TTL sweep shrinks it.
    pub fn outstanding_challenges(&self, now: &str) -> Result<i64, Error> {
        let conn = self.conn.lock().unwrap();
        let count = conn.query_row(
            "SELECT COUNT(*) FROM challenges WHERE consumed_at IS NULL AND expires_at >= ?1",
            params![now],
            |row| row.get(0),
        )?;
        Ok(count)
    ...
    /// Consume a challenge, returning false when it is unknown, spent,
    /// expired, or was issued for a different purpose. The single
    /// `UPDATE ... WHERE consumed_at IS NULL` row count is the
    /// race-free single-use check — two concurrent sessions presenting
    /// the same challenge serialize here, and the loser is refused.
    pub fn claim_challenge(&self, challenge: &str, purpose: &str, now: &str) -> Result<bool, Error> {
        let conn = self.conn.lock().unwrap();
        let updated = conn.execute(
            "UPDATE challenges SET consumed_at = ?3
             WHERE challenge = ?1 AND purpose = ?2
               AND consumed_at IS NULL AND expires_at >= ?3",
            params![challenge, purpose, now],
        )?;
        Ok(updated == 1)
    }

    // ─── Recall writes (propagation guard) ───────────────────────────

    /// Record the state of the last write Google accepted for a
    /// binding, so reconciliation can recognize a read that merely lags
    /// it (profile §4 requirement 5).
    pub fn record_recall_write(
        &self,
        device_binding: &str,
        case_open: bool,
        banned: bool,
        ...
        let conn = self.conn.lock().unwrap();
        conn.execute(
            "INSERT INTO recall_writes (device_binding, case_open, banned, written_at)
             VALUES (?1, ?2, ?3, ?4)
             ON CONFLICT(device_binding) DO UPDATE
                SET case_open = ?2, banned = ?3, written_at = ?4",
            params![device_binding, case_open as i32, banned as i32, now],
        )?;
        Ok(())
    }

    /// The last accepted write for a binding: (case_open, banned,
    /// written_at).
    pub fn last_recall_write(
        &self,
        device_binding: &str,
    ) -> Result<Option<(bool, bool, String)>, Error> {
        let conn = self.conn.lock().unwrap();
        let row = conn
            .query_row(
                "SELECT case_open, banned, written_at FROM recall_writes
                 WHERE device_binding = ?1",
    ...
    /// `deviceRecall:write`, successful or not — a refused write is as
    ...
    pub fn append_write_log(
        ...
        Self::append_write_log_conn(
            &conn, device_binding, authorized_by, case_open, banned, fields_written, outcome, now,
        )
    }
    ...
    fn append_write_log_conn(
        ...
        let preimage = format!(
            "{previous_hash}|{now}|{device_binding}|{authorized_by}|{case_open}|{banned}|\
             {fields_written}|{outcome}"
        );
        let entry_hash = util::sha256_hex(preimage.as_bytes());

        conn.execute(
            "INSERT INTO write_log
             (recorded_at, device_binding, authorized_by, case_open, banned, fields_written,
        ...
        Ok(WriteLogEntry {
            ...
            case_open,
            banned,
            fields_written: fields_written.to_string(),
    ...
    pub fn write_log(&self, limit: i64) -> Result<Vec<WriteLogEntry>, Error> {
        ...
        let mut statement = conn.prepare(
            "SELECT sequence, recorded_at, device_binding, authorized_by, case_open, banned,
                    fields_written, outcome, previous_hash, entry_hash
...
mod tests {
    ...
    fn write_log_chains_and_verifies() {
        let store = Store::in_memory().unwrap();
        store
            .append_write_log("d1", "verdict-a", false, true, "{\"bitSecond\":true}", "ok", "2026-08-08T00:00:00Z")
            .unwrap();
        store
            .append_write_log("d1", "expiry", false, false, "{\"bitSecond\":false}", "ok", "2026-08-09T00:00:00Z")
            .unwrap();
    ...
    fn temp_path(name: &str) -> std::path::PathBuf {
        let path = std::env::temp_dir()
            .join(format!("onym-android-store-{name}-{}.sqlite", std::process::id()));
    ...
    /// Re-opening an existing store must be a no-op — `migrate()` runs
    /// on every boot.
    #[test]
